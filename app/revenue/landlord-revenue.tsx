...
export default function LandlordRevenueDashboard() {
  ...
  const exportCSV = async () => {
    try {
      ...
      if (error) throw error;
      Alert.alert('Success', 'Revenue report exported');
    } catch (error: any) {
      Alert.alert('Error', error.message);
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>My Revenue</Text>
      ...
      <View style={styles.statsGrid}>
        <View style={styles.statCard}>
          <Text style={styles.statValue}>${data?.summary?.landlordRevenue?.toFixed(2) || '0'}</Text>
          <Text style={styles.statLabel}>Your Earnings</Text>
        </View>
        <View style={styles.statCard}>
          <Text style={styles.statValue}>{data?.summary?.transactionCount || 0}</Text>
      ...
      <View style={styles.transactionsSection}>
        ...
        {data?.transactions?.slice(0, 10).map((tx: any) => (
          <View key={tx.id} style={styles.txCard}>
            ...
            <Text style={styles.txAmount}>${parseFloat(tx.landlord_revenue).toFixed(2)}</Text>
          </View>
        ))}
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  exportBtn: { backgroundColor: '#007AFF', padding: 12, borderRadius: 8 },
  ...
  txCard: { backgroundColor: '#fff', padding: 16, borderRadius: 12, marginBottom: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  txLeft: { flex: 1 },
  txType: { fontSize: 16, fontWeight: '600', color: '#1a1a1a', textTransform: 'capitalize' },
  txDate: { fontSize: 14, color: '#666', marginTop: 4 },
  txAmount: { fontSize: 18, fontWeight: '700', color: '#34C759' }
});
